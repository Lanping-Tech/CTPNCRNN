...
def get_bboxes_crnn(gt_path, im_scale, im_shape):
    polys = []
    with open(gt_path, 'r',encoding='utf-8') as f:
        lines = f.readlines()
    tag = []
    ...
    for ii in range(len(polys)):
        polys_t = polys[ii]
        x_min = np.min(polys_t[:, 0])
        y_min = np.min(polys_t[:, 1])
        x_max = np.max(polys_t[:, 0])
        y_max = np.max(polys_t[:, 1])
        res_polys.append([x_min, y_min, x_max, y_max])
    
    return np.array(res_polys), tag

def iou_crnn(box1,box2):
